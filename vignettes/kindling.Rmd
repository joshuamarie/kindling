...
As of `{kindling}` 0.3.x, it supports most of NN architectures thanks to its versatility, but it doesn't support:

1.  Residual Networks (ResNet)
2.  Automatic Integration (AutoInt)
3.  Self-Attention and Inter-sample Attention Transformer (Saint)

To use all of these, use `{brulee}` (>= 1.0.0) to use the said NN architectures instead. 
