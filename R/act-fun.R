...
parse_activation_spec = function(activations, n_layers) {
    if (is.null(activations)) {
        ...
        elem_names = names(activations) %||% rep("", length(activations))
        parsed_activation =
            purrr::pmap(list(seq_along(activations), activations, elem_names), function(pos, elem, elem_name) {
                ...
                if (elem_name != "" && is.list(elem)) {
                    ...
                    cli_abort(c(
                        "Invalid activation specification at position {pos}.",
